...
> **Related — `@circuit-llm/mcp`:** a runnable MCP server that exposes Circuit's data + agent-swarm
> intelligence as x402-paid tools for any AI agent. It builds on `@circuit-llm/data` + `@circuit-llm/wallet`
> but lives in its own repo: **[Circuit-LLM/MCP](https://github.com/Circuit-LLM/MCP)**.
